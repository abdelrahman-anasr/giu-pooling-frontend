...
export default function RideResultCard({tripId , tripLocation , fromToGiu , girlsOnly , driversName , departureTime , seatsLeft , carType , basePrice , percentTop , percentLeft , requestFunction}) 
{
    const girlsOnlyString = girlsOnly ? 'Girls-Only' : 'Not Girls-Only';
    const fromToGiuString = fromToGiu ? 'From GIU' : 'To GIU';
    return (
        <div style={{ width: 880, height: 240, left: percentLeft, top: percentTop + '%', position: 'absolute', background: 'white', borderRadius: 20, border: '4px black solid'}}>
        ...
        <ApproveButton label={'Book'} functionToCall={() => {requestFunction(tripId)}} topPosition={140} leftPosition={630} size={'small'}/>
        </div>
    );
}